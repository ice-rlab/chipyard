      ...
      clock_info(clock_info)
{
  ...
  fprintf(stdout, "Tracedoctor constructor\n");
  ...
  for (auto &arg: args) {
    if (arg.find(traceworker_arg) == 0) {
      ...
      if (workerargs.empty()) {
        fprintf(stdout, "TraceDoctor@%d invalid worker argument", info.tracerId);
      ...
      if (reg == tracedoctor_register.end()) {
        fprintf(stdout, "TraceDoctor@%d unknown worker %s", info.tracerId, workername);
        // TODO: Tracedoctor argument: <F0>^E<E7>^A
        // TraceDoctor@0 unknown worker ^P<9B>^H^B
